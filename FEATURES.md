...
- `app/page.tsx` owns the fullscreen state. It calls the browser Fullscreen API
  on enter and `exitFullscreen()` on exit, with a **fallback** that swaps the
  hero for a `bg-black/80 h-screen` chrome-less expanded layout when the API is
  unavailable. The hero's text overlay is hidden while fullscreen.
...
- `lib/fullscreen.test.ts` (new) — 7 unit tests for the pure logic.
...
- The `?fullscreen=` param is a single on/off flag; a future cycle could layer
  it behind the galaxy params like the other toggles.
